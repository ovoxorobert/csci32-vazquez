...
async function seedProducts(prisma) {
    ...
    await prisma.product.createMany({
        data: [
            {
                name: 'Labubu',
                description: '3D printed Labubu figure fits inside all standard PC cases.',
                price: 199.99,
                stock: 50,
                sku: 'QTL-001',
            },
...
async function seedPostsAndComments(prisma) {
    ...
    if (!author) {
        console.warn('⚠️ No users found. Skipping Post seed (needs at least one user to own posts).');
        return;
    }
